...
import { createStorage, setLevelStars, getTotalStars } from '../utils/storage.js';
...
export default class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.audio('sfx-star', 'audio/sfx-star.wav');
    this.load.audio('sfx-boss-pickup', 'audio/sfx-boss-pickup.wav');
    this.load.audio('sfx-level-clear', 'audio/sfx-level-clear.wav');
  ...
  create() {
    ...
    const totalStars = getTotalStars(createStorage());
    const unlocked = unlockedCycles(totalStars);
    const savedId = createStorage().get(SELECTED_KEY);
    const cycle = unlocked.find((c) => c.id === savedId) || activeCycle(totalStars);
  ...
  handlePokemonPickup(mon) {
    ...
    const sfxKey = shiny ? 'sfx-shiny' : isBoss ? 'sfx-boss-pickup' : 'sfx-pickup';
    this.safePlay(sfxKey, { volume: 0.75 });
